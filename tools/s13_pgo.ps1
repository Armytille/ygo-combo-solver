# PGO session 13 — MEME mecanique que s12_pgo.ps1, avec :
#   * temoin nomme combosolver_lto_s13.exe (NE PAS ecraser le temoin s12,
#     conserve pour l'archeologie des mesures) ;
#   * un 4e run d'entrainement AVEC options (--adapt + --options 256) pour
#     couvrir le mineur v3, la garde ctx et le chemin des macros — le second
#     pipeline s12 etait deja "options comprises".
#
...
# 1. temoin LTO du code s13
Copy-Item bin\Release\combosolver.exe bin\Release\combosolver_lto_s13.exe -Force
...
# 3. entrainement : les QUATRE regimes (LDS, tirages, finisseur, options)
& .\bin\Release\combosolver.exe $ref `
    --scriptdir ..\deps\scripts_2026-04-13\script `
    --solve --solve-ms 60000 `
    --outdir s13_pgo_train1 --no-chain Zalen --no-chain "Crystal Wing" `
    *> s13_pgo_train1.log
...
& .\bin\Release\combosolver.exe $ref `
    ...
    --outdir s13_pgo_train2 *> s13_pgo_train2.log
...
& .\bin\Release\combosolver.exe $ref `
    ...
    --solve-ms 30000 --seed $Seed `
    --no-chain Zalen --no-chain "Crystal Wing" `
    --outdir s13_pgo_train3 *> s13_pgo_train3.log
...
& .\bin\Release\combosolver.exe $ref `
    ...
    --resolve "PSY-Framelord Omega@terrain:2" `
    --resolve "Trishula, Dragon of the Ice Barrier@terrain" `
    --outdir s13_pgo_train4 *> s13_pgo_train4.log
...
Write-Output "--- exe PGO lie (temoin : combosolver_lto_s13.exe)"
